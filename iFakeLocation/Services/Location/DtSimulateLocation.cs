...
/// <summary>Simulates location via the com.apple.dt.simulatelocation lockdown service (iOS &lt; 17).</summary>
internal sealed class DtSimulateLocation(DeviceRecord device) : LocationService(device) {
    ...
    public override void SetLocation(PointLatLng? target) {
        ...
        try {
            var err = idevice.idevice_new_with_options(out deviceHandle, _device.Udid,
                (int)(_device.IsNetwork ? iDeviceOptions.LookupNetwork : iDeviceOptions.LookupUsbmux));
