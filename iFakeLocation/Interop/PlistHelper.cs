...
internal static class PlistHelper {
    ...
    public static Dictionary<string, object?> ReadPlistDictFromStream(Stream stream, ICollection<string>? keys = null) {
        var nsObject = PropertyListParser.Parse(stream);
        return ReadPlistDictFromNSObject(nsObject, keys);
    }

    public static Dictionary<string, object?> ReadPlistDictFromString(string str, ICollection<string>? keys = null) {
